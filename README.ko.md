...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code)용 구조화된 계획 플러그인입니다. 단계(이해, 탐색, 대안, 초안, 비판, 수정)를 거치며 각 단계마다 검증을 수행해 고품질 구현 계획서를 만들어냅니다.
...
| **탐색** | 코드베이스를 읽고 파일/아키텍처/패턴을 나열. 이전 세션 학습 주입 (Reflexion). | 파일 경로 참조 2+. 계획 헤딩이 없어야 함. |
...
| **비판 (×2)** | 12개 원칙 (P1-P12)에 대해 PASS/FAIL 평가. 관점 회전: 기술적 → 유지보수성. | `<promise>` 태그 없어야 함. 번호 항목 3+, 원칙 증거 6+ (P-번호 + PASS/FAIL 합산). |
...
| **Breaking changes** | 단계별 분석 + 영향받는 호출 사이트 명시 | 간략한 언급 |
...
├── hooks/
│   ├── hooks.json      # 훅 등록 (stop, pretooluse)
